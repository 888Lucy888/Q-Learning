...
from learning.QlearningTom import QlearningTom  # Renamed from QlearningTom
...
qlearning_tom = QlearningTom(width, height, config_file='config/tomqlearning.json')  # Changed to use QlearningTom
...
if load_weights:
    ...
    qlearning_tom.model.load_state_dict(torch.load('Checkpoint/dqn_gato.pth'))  # Load DQN model for Tom
...
for episode in range(max_episodes):
    ...
    while run and steps < max_steps:
        ...
        action_jerry = qlearning_jerry.take_action(jerry.get_state(BLOCK_SIZE, width), epsilon_jerry, queso_actual)
        ...
        reward_jerry = jerry.move(action_jerry, nivel, BLOCK_SIZE, width, height)
        reward_tom = tom.move(action_tom, nivel, BLOCK_SIZE, width, height)
        ...
        qlearning_tom.update_q_table(tom_state, action_tom, reward_tom, next_state_tom)  # DQN updates model
